...
async def list_active_services_filtered(
    session: AsyncSession,
    hidden_categories: tuple[str, ...] | set[str] | list[str],
) -> list[Service]:
    query = select(Service).where(Service.is_active == True)
    hidden = [c for c in hidden_categories if c]
    if hidden:
        query = query.where(~Service.category.in_(hidden))
    return (await session.execute(
...
async def list_active_services_by_category_filtered(
    session: AsyncSession,
    category: str | None,
    hidden_categories: tuple[str, ...] | set[str] | list[str],
) -> list[Service]:
